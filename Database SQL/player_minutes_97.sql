SELECT *
FROM
(SELECT Player.name, Player.birthdate, (all_start_minutes.minutes+all_subin_minutes.minutes-all_subout_minutes.minutes) as minutes
FROM Player,
...
WHERE Player.id = all_start_minutes.id AND Player.id = all_subin_minutes.id AND Player.id = all_subout_minutes.id AND (Player.nat1 = 'United States' OR Player.nat2 = 'United States') AND Player.birthdate >= '1997-01-01') as m
WHERE minutes > 0
